...
Object.keys(requiredScores).forEach(category => {
  ...
  if (actualBestOutOf100 < requiredScores[category]) {
    ...
    prComment.push(
      `<h3>❌ ${category}: ${actualBestOutOf100}/${requiredOutOf100}</h3>`
    );
    success = false;
  } else {
    ciStdout.push(`✅ ${category}: ${actualBestOutOf100}/${requiredOutOf100}`);
  ...
  if (!areAllScoresEqual) {
    ...
    prComment.push(
      `<p>Scores varied across runs: [${scoresAcrossRunsByCategory[
        category
      ].join(" ")}]</p>`
    );
  }
...
prComment.push("<h2>Detailed reports</h2>");
prComment.push("<p>");
const reportLinks = htmlReportFilenames.map((filename, idx) => {
