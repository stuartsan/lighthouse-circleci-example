...
class LoadAudit extends Audit {
  static get meta() {
    return {
      ...
      failureTitle: `JS bundle exceeds your threshold of ${process.env.MAX_BUNDLE_SIZE_KB}kb`,
  ...
  static async audit(artifacts, context) {
		...
		const belowThreshold = bundleRecord.transferSize <= process.env.MAX_BUNDLE_SIZE_KB * 1024;

		return {
			rawValue: (bundleRecord.transferSize / 1024).toFixed(1),
			// Cast true/false to 1/0
			score: Number(belowThreshold),
			displayValue: `${bundleRecord.url} was ${(bundleRecord.transferSize / 1024).toFixed(1)}kb`,
		};
	}
}
